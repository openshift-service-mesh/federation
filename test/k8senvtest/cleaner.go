	...
	for _, o := range objects {
		...
		if namespace, ok := obj.(*corev1.Namespace); ok {
			...
			for _, gvk := range c.namespacedGVKs {
				...
				errDelete := c.client.DeleteAllOf(context.Background(), &u, client.InNamespace(namespace.Name))
				Expect(client.IgnoreNotFound(ignoreMethodNotAllowed(errDelete))).ShouldNot(HaveOccurred())
			}

			Eventually(func() error {
				key := client.ObjectKeyFromObject(namespace)

				if errGet := c.client.Get(context.Background(), key, namespace); errGet != nil {
...
func lookupNamespacedResources(clientset *kubernetes.Clientset) map[string]schema.GroupVersionKind {
	...
	_, apiResources, errList := clientset.DiscoveryClient.ServerGroupsAndResources()
	if errList != nil {
		panic(errList)
	}
	...
	for _, apiResourceList := range apiResources {
		...
		defaultGV, errParse := schema.ParseGroupVersion(resources.GroupVersion)
		Expect(errParse).ShouldNot(HaveOccurred())
